...
def show_memory_leaks(
    ...
    if refs_depth == 0 and back_refs_depth == 0:
        return
...
class MemoryUsageData(object):
    ...
    Call ``memory_usage_data.graph_leaks()`` from a view and enable the
    appropriate waffle switch(es) to start generating memory leak diagnostic
    information:
    ...
    * monitoring_utils.create_memory_graphs - Also generate reference graphs
    ...
    pass it the ``--nothreading`` option since the ``objgraph`` library isn't
    thread-safe.  (In devstack, do this in docker-compose.yml.)
    """
